...
-- Function to set first admin automatically
...
begin
    ...
    -- Only set first admin if no admins exist
    if existing_count = 0 then
        ...
        -- If user exists, make them super_admin
        if found then
...
begin
    ...
    values (p_user_id, p_email, p_role, auth.uid())
...
grant execute on function public.set_first_admin() to authenticated;
